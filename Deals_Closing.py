...
def register_deals_closing_callbacks(app):
    ...
    def update_dashboard(selected_owner, selected_service, n_clicks):
        filtered_df = df.copy()
        ...
        for kpi, value in zip(
            ...
            [total_row['# Deals Entered'], total_row['# Deals Closed'], grouped.iloc[-1]['% Deals Closed']]
        ):
            kpi_cards.append(
                html.Div([
        ...
        if ctx.triggered and ctx.triggered[0]['prop_id'].startswith('export-btn-deals-closing'):
            csv_data = dcc.send_data_frame(grouped.to_csv, "Deals_Closing_Export.csv")

        return kpi_cards, table, dcc.Graph(figure=bar_fig), csv_data
