...
df["Invoice_Amount"] = pd.to_numeric(df["Invoice_Amount"], errors="coerce").abs().fillna(0)
...
layout = html.Div(style={"backgroundColor": "black", "color": "white"}, children=[
    ...
    html.Div(style={"display": "flex", "gap": "10px", "marginBottom": "20px"}, children=[
        ...
        dcc.Dropdown(
            id="entity_filter",
            options=[{"label": e, "value": e} for e in sorted(df["Entity"].dropna().unique())],
    ...
    dash_table.DataTable(
        ...
        columns=[
            ...
            {"name": "Invoice Amount", "id": "Invoice_Amount", "type": "numeric",
             "format": {"locale": {"symbol": ["$", ""]}, "specifier": "$,.2f"}}
        ],
        page_size=20,
        style_table={"overflowX": "auto"},
        style_header={"backgroundColor": "#222", "color": "white", "fontWeight": "bold"},
        style_cell={"backgroundColor": "#111", "color": "white", "padding": "10px", "textAlign": "left"},
...
def update_table(year, month, entity, mp_code, user_data):
    ...
    dff = dff[["MP", "Name", "Description", "Invoice Date", "Invoice_Amount"]].copy()
    dff["Invoice Date"] = dff["Invoice Date"].dt.strftime("%Y-%m-%d")

    total_amount = dff["Invoice_Amount"].sum()
    return dff.to_dict("records"), f"Total Invoice Amount: ${total_amount:,.2f}"
...
def export_table(n_clicks, table_data):
    ...
    return dict(content=csv_data, filename="invoice_data.csv", type="text/csv")
