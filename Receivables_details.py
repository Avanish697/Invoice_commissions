...
df['Receivables'] = df.apply(
    lambda row: float(row['Invoice_Amount']) if row['Status'] == 'AUTHORISED' and pd.isna(row['FullyPaidOnDate']) else 0,
    axis=1
)

df_display = df[['Location', 'Name', 'Description', 'Due Date', 'Days Overdue', 'Receivables', 'Year', 'Month', 'Entity']].copy()
df_display['MP'] = df_display['Location']
df_display = df_display[['MP', 'Name', 'Description', 'Due Date', 'Days Overdue', 'Receivables', 'Year', 'Month', 'Entity']]
...
layout = html.Div(style={"backgroundColor": "black", "color": "white"}, children=[
    ...
    html.Div(style={"display": "flex", "gap": "10px", "marginBottom": "20px", "flexWrap": "wrap"}, children=[
        ...
        dcc.Dropdown(
            id='entity-filter',
            options=[{'label': e, 'value': e} for e in sorted(df_display['Entity'].dropna().unique())],
        ...
        dcc.Dropdown(
            id='mp-filter',
            options=[],  # Initially empty, will be updated based on logged-in user
    ...
    dash_table.DataTable(
        ...
        columns=[
            ...
            {"name": "Receivables", "id": "Receivables", "type": "numeric",
             "format": {"locale": {"symbol": ["$", ""]}, "specifier": "$,.2f"}}
        ],
        page_size=20,
...
@callback(
    ...
    Input('user-store', 'data')  # Added for filtering by logged-in user
)
def update_mp_filter(user_data):
    username = user_data.get("username")
    # Filter the MP options based on the logged-in user's MP
    if username != 'admin':
...
def update_receivables(year, month, entity, mp, user_data):
    ...
    return display_data.to_dict('records'), f"Total Receivables: ${total_value:,.2f}"
